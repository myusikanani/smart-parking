...
import { Zap, Accessibility, Car, Bike, Check, RefreshCw, Sparkles, Layers } from 'lucide-react';
...
export const InteractiveFloorMap: FC<InteractiveFloorMapProps> = ({
  ...
  const getStatusColor = (status: string, isSelected: boolean) => {
    if (isSelected) return 'border-cyan-400 bg-cyan-500/20 text-cyan-300 ring-2 ring-cyan-400/50 shadow-lg shadow-cyan-500/20';
    switch (status) {
      case 'available':
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-[var(--border)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--text-secondary)]">Click on any available green spot to choose your parking bay</p>
          </div>
        </div>
            ...
            {topRowSlots.map((slot) => {
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl border transition-all duration-200 relative flex flex-col justify-between min-h-[100px] ${getStatusColor(
                    slot.status,
                    isSelected
                  )}`}
                >
                  ...
                  <div className="my-1">
                    <span className="text-[10px] block opacity-80 capitalize">{slot.category.replace('-', ' ')}</span>
                    <span className="font-semibold text-xs">₹{slot.pricePerHour}/hr</span>
                  </div>
